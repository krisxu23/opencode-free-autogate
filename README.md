...
| **IP 信誉体检** | 只体检正式池节点（转正时查一次 + 启动补查，缓存 7 天）：AbuseIPDB 滥用举报 + IPinfo 地区/ASN + Spamhaus DQS 黑名单三源交叉，映射 A/B/C/D 分级；fail-open——缺 key 跳过该源、服务故障不剔除节点，信誉只影响出场顺序绝不单独判死 |
...
| `IPINFO_TOKEN` / `ABUSEIPDB_KEY` / `SPAMHAUS_DQS_KEY` | 空 | IP 信誉体检凭据（均可选）：缺哪个跳过哪个源；AbuseIPDB 免费 1000 次/天、IPinfo 免费 5 万次/月，正式池体检配额绰绰有余 |
