...
| 功能 | 说明 |
|---|---|
| **两轮检测** | GET 全量初检（零配额）＋ chat 深检复检（动态数量），普通代理与 sing-box 高级映射节点**合并进同一条初检队列**按「检测并发」一路测完；初检一通过立即流入深检（流水线，不等整轮）；启动即跑、每小时整体复检 ±20% 抖动 |
...
| **在线节点池** | 定时拉取源链接 → 真实探活 → 健康入池 / 失效自删；**新候选全量探活、健康即入池，测试与入池均无数量上限**——规模完全由你的源列表决定，按「检测并发」一轮一轮测到完为止；源直连失败时自动经健康出口兜底重试；支持文本列表、JSON、base64 订阅、裸 socks5/ip:port 列表 |
